...
| F-013 | Node functions not deployed: dir-casing + missing edge files (CONFIRMED) | config/deploy | High | Phase 2 · **IMPLEMENTED + VERIFIED 2026-07-01** |
...
**IMPLEMENTED + VERIFIED — 2026-07-01 (first Wave-2 item shipped).** The dir-casing half is fixed and confirmed working in production. Sequence of the actual fix:
- **What triggered it:** the hourly `lift-wip-sync` LIFT feed had gone silent ~15h (the new freshness badge caught it — see WORKPLAN 2026-07-01 entry). Diagnosis walked exactly the F-013 path: function callable on-demand (manual GET wrote snapshot 39 cleanly, proving code + LIFT endpoint healthy), but zero *scheduled* invocations and an empty function log ⇒ the cron was never registering, isolated to scheduled-function discovery failing on the capital-F dir.
- **The fix:** `git mv netlify/Functions → netlify/functions` via a **two-hop rename through a temp name** (`Functions` → `functions-tmp` → `functions`) because Windows is case-insensitive and a direct case-only `git mv` no-ops (git doesn't record the change, so it never propagates to Netlify's case-sensitive Linux build). Then `netlify.toml` `functions =` updated capital→lowercase. Commit `7cc3b00`.
- **VERIFIED (the real proof, not the deploy log):** after deploy, `sched_snapshots` produced **unattended** `LIFT API (auto feed)` rows at the top of the hour that nobody triggered manually (confirmed by DB-clock `age` under an hour). A clean deploy alone does NOT prove the cron fires — an on-demand endpoint hit exercises the function, not the schedule — so the top-of-hour unattended row was the deciding evidence.
- **Byproduct win:** `lock-wip`'s Saturday cron (`0 5 * * 6`) lived in the same capital-F dir, so it was dead for the same reason and is now able to register. (It's still slated for *deletion* under F-003, not preservation — the registration fix just means it's no longer a silent-failure mystery in the meantime.)

⚠️ **CORRECTION to the WORKPLAN 2026-06-30 entry.** That entry claims a "Netlify functions-dir CASING FIX" was applied and states the change as `functions = "netlify/functions"` → `"netlify/Functions"` — i.e. **backwards** (lowercase→capital, the wrong direction) and **apparently never actually committed**, since the toml was still capital-F when inspected on 2026-07-01. Treat the 6/30 casing claim as erroneous; the 2026-07-01 fix above is the real, verified one.

**STILL PENDING (not done 2026-07-01):** only the dir-casing half shipped. The bundled cleanups this finding calls for — deleting `F-012` (`claude.mjs`), `F-003` (`lock-wip` cron+fn), `F-017` (`generate-pdf.mjs`), and removing the bogus `[[edge_functions]]` decls for `/api/lock-wip` + `/api/generate-pdf` — were NOT touched and remain open Wave-2 work.
